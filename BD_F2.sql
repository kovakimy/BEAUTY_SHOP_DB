...
CREATE OR REPLACE FUNCTION DELETE_SPECIALIST_BY_NAME(NAME_TO_DELETE TEXT)
	RETURNS VOID AS
	$$
	BEGIN
	IF NOT EXISTS (SELECT S.NAME_ 
				   FROM SPECIALISTS S 
				   WHERE S.NAME_ = NAME_TO_DELETE) THEN
						RAISE EXCEPTION 'Illegal parameter: %', NAME_TO_DELETE
      					USING HINT = 'Check your NAME';
	END IF;
  	DELETE FROM SPECIALISTS SPECIALIST
		WHERE SPECIALIST.NAME_ = NAME_TO_DELETE;
	END;
	$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION FIND_CLIENT_BY_NAME(NAME_TO_FIND TEXT)
	RETURNS INT AS
	$$
	BEGIN
	IF NOT EXISTS (SELECT C.NAME_ 
				   FROM CLIENTS C 
				   WHERE C.NAME_ = NAME_TO_FIND) THEN
						RAISE EXCEPTION 'Illegal parameter: %', NAME_TO_FIND
      					USING HINT = 'Check your NAME';
	END IF;
	RETURN (SELECT C.ID 
				   FROM CLIENTS C 
				   WHERE C.NAME_ = NAME_TO_FIND);

	END;
	$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION DELETE_CLIENT_BY_NAME(NAME_TO_DELETE VARCHAR(12))
	RETURNS VOID AS
	$$
	BEGIN
	IF NOT EXISTS (SELECT C.NAME_ 
				   FROM CLIENTS C 
				   WHERE C.NAME_ = NAME_TO_DELETE) THEN
						RAISE EXCEPTION 'Illegal parameter: %', NAME_TO_DELETE
      					USING HINT = 'Check your NAME';
	END IF;
	DELETE FROM DISCOUNT_CARDS CARD_
		WHERE CARD_.CLIENT_ID = (SELECT FIND_CLIENT_BY_NAME(NAME_TO_DELETE));
  	DELETE FROM CLIENTS CLIENT
		WHERE CLIENT.NAME_ = NAME_TO_DELETE;

	END;
	$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION DELETE_RECORD_BY_DATE(DATE_TO_DELETE DATE)
	RETURNS VOID AS
	$$
	BEGIN
	IF NOT EXISTS (SELECT R.DATE_ 
				   FROM RECORDS R 
				   WHERE R.DATE_ = DATE_TO_DELETE) THEN
						RAISE EXCEPTION 'Illegal parameter: %', DATE_TO_DELETE
      					USING HINT = 'Check your ID';

	END IF;
  	DELETE FROM RECORDS RECORD
		WHERE RECORD.DATE_ = DATE_TO_DELETE;
	END;
	$$ LANGUAGE plpgsql;